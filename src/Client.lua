...
function Client:Create()
  ...
  -- Remote messages
  NotaLoot:RegisterCallback(NotaLoot.MESSAGE.OPEN_CLIENT, function(_, _, sender)
    client:OnShowRequest(sender)
  end)
  NotaLoot:RegisterCallback(NotaLoot.MESSAGE.ADD_ITEM, function(_, _, sender, index, encodedItem)
    client:OnAddItem(sender, tonumber(index), NotaLoot.Item:Decode(encodedItem))
  end)
  NotaLoot:RegisterCallback(NotaLoot.MESSAGE.ASSIGN_ITEM, function(_, _, sender, index, winner, isByRandom)
    local isByRandom = (isByRandom == "true")
    client:OnAssignItem(sender, tonumber(index), winner, isByRandom)
  end)
  NotaLoot:RegisterCallback(NotaLoot.MESSAGE.DELETE_ITEM, function(_, _, sender, index)
    client:OnDeleteItem(sender, tonumber(index))
  end)
  NotaLoot:RegisterCallback(NotaLoot.MESSAGE.DELETE_ALL_ITEMS, function(_, _, sender)
    client:OnDeleteAllItems(sender)
  end)
  NotaLoot:RegisterCallback(NotaLoot.MESSAGE.SYNC_RESPONSE, function(_, _, sender, encodedItems)
...
function Client:CreateSession(owner, notify)
  ...
  session:RegisterCallback(NotaLoot.MESSAGE.BID_ITEM, function(_, ...)
    self:OnBidItem(...)
  end)
  session:RegisterCallback(NotaLoot.MESSAGE.ON_CHANGE, function(_, session, index)
