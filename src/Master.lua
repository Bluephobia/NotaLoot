...
function Master:Create()
  ...
  -- Remote messages
  NotaLoot:RegisterCallback(NotaLoot.MESSAGE.INIT, function(_, _, sender, version)
    master:OnRemoteInit(sender, version)
  end)
  NotaLoot:RegisterCallback(NotaLoot.MESSAGE.BID_ITEM, function(_, _, sender, index, bid, ...)
    master:OnBidRequest(sender, tonumber(index), tonumber(bid), ...)
  end)
  NotaLoot:RegisterCallback(NotaLoot.MESSAGE.SYNC_REQUEST, function(_, _, sender)
    master:OnSyncRequest(sender)
  end)
  NotaLoot:RegisterCallback(NotaLoot.MESSAGE.VIEW_REQUEST, function(_, _, sender, view)
    view = (view == "true")
    master:OnViewRequest(sender, view)
  end)
  NotaLoot:RegisterCallback(NotaLoot.MESSAGE.VIEW_RESPONSE, function(_, _, sender, granted, encodedBids)
...
function Master:ConfigureItemRow(row, item)
  ...
  row:RegisterCallback(NotaLoot.MESSAGE.DELETE_ITEM, function() self.session:RemoveItem(item) end)
end
...
function Master:CreateSession()
  ...
  session:RegisterCallback(NotaLoot.MESSAGE.ADD_ITEM, function(_, _, index, item)
    self:OnAddItem(index, item)
  end)
  session:RegisterCallback(NotaLoot.MESSAGE.BID_ITEM, function(_, _, item)
    self:ReloadItem(item)
  end)
  session:RegisterCallback(NotaLoot.MESSAGE.ASSIGN_ITEM, function(_, _, item, winner, isByRandom)
    self:OnAssignItem(item, winner, isByRandom)
  end)
  session:RegisterCallback(NotaLoot.MESSAGE.DELETE_ITEM, function(_, _, index)
    self:OnDeleteItem(index)
  end)
  session:RegisterCallback(NotaLoot.MESSAGE.ON_CHANGE, function(_, _, ...)
...
function Master:OnLootMessage(text, ...)
  ...
  local itemLink = text:match(LOOT_ITEM_SELF:sub(1, -4)..".*(|c%x+|Hitem:.+|r)")
  if not itemLink then return end

  local item = NotaLoot.Item:CreateForLink(itemLink)
